...
public class TaskService : ITaskService
{
    ...
    public async Task<TaskMessage[]> GetComments(long taskId, CancellationToken token)
    {
        ...
        const int messageCountToGet = 5;

        var cachedTaskMessages = await _distributedCache.GetStringAsync(cacheKey, token);

        if (!string.IsNullOrEmpty(cachedTaskMessages))
        {
            return JsonSerializer.Deserialize<TaskMessage[]>(cachedTaskMessages) ?? throw new ArgumentException();
        }

        var taskMessages = await _taskCommentRepository.Get(
